...
bayerPatternTL = img(1:2,1:2);% bayer pattern top left
Pgrbg = dot(bayerPatternTL(:),grbg(:));
Prggb = dot(bayerPatternTL(:),rggb(:));
Pbggr = dot(bayerPatternTL(:),bggr(:));
Pgbrg = dot(bayerPatternTL(:),gbrg(:));
...
img4 = zeros(size(img,1), size(img,2));
img_rggb = zeros(size(img,1), size(img,2), 3);
% img_rggb(2:2:end,1:2:end,2) = img(2:2:end,1:2:end);
img_rggb(1:2:end,1:2:end,1) = img(1:2:end,1:2:end);
img_rggb(2:2:end,2:2:end,3) = img(2:2:end,2:2:end);
img_rggb(1:2:end,2:2:end,2) = img(1:2:end,2:2:end);

temp = [Pgrbg, Prggb, Pbggr, Pgbrg];
[A, I] = max(temp);
switch I
    case 1
        disp('The best bayer pattern is grbg.');
%         img = img_rgn;
        figure, imshow(img); title('img grbg');
    case 2
        disp('The best bayer pattern is rggb.');
        img = img_rggb;
        figure, imshow(img); title('img rggb');        
    case 3
        disp('The best bayer pattern is bggr.');
%         img = img_rgb;
        figure, imshow(img); title('img bggr');    
    case 4
        disp('The best bayer pattern is gbrg.');     
%         img = img_rgb;
...
imwrite(img_w, 'gray_img100.jpeg', 'Quality', 100);
