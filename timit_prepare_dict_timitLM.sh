...
(echo '!SIL SIL'; echo '<UNK> SIL' ) | \
 cat - $dir/lexicon1.txt  > $dir/lexicon.txt || exit 1;
...
# modified to build bigram LM from our data (normal + ALS)
  cut -d' ' -f2- conf/train.text | sed -e 's:^:<s> :' -e 's:$: </s>:' \
    > $srcdir/lm_train.text
  build-lm.sh -i $srcdir/lm_train.text -n 2 -o $tmpdir/lm_phone_bg.ilm.gz
...
# modified to build trigram LM from our data (normal + ALS)
  cut -d' ' -f2- conf/train.text | sed -e 's:^:<s> :' -e 's:$: </s>:' \
    > $srcdir/lm_train.text
  build-lm.sh -i $srcdir/lm_train.text -n 3 -o $tmpdir/lm_phone_tg.ilm.gz

  compile-lm $tmpdir/lm_phone_tg.ilm.gz -t=yes /dev/stdout | \
  grep -v unk | gzip -c > $lmdir/lm_phone_tg.arpa.gz 
